...
- The outline is inset by a 2-point `Gap`, so that two pieces side by side keep a gap, and drawn with `Painter2D` as one shape in the rarity's `Surface` tone, with a line running inside it (pink while hovered).
