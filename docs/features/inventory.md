...
- Motion: taken in the hand the piece shrinks a little around the point grabbed (`Hold`, class `artifact-piece--held`); put down it grows back past its size and settles (`Land`, played by `TetrisInventory` on the item just added, `ease-out-back`); dropped on a grid where it doesn't fit, it goes back home and shakes, its outline in the accent color (`LandRefused`, `TetrisInventory.AddItem(..., refused: true)`, `artifact-piece--refused`, `RefuseShake`), while dropped outside the grids it just goes back; turned it swings a quarter turn around the same point, which stays under the pointer (`PlayTurn`, called by `InventoryDrag`). The grid places and turns the piece around its bottom left corner, so the shrink and the shake are on an inner `artifact-piece__body` and the swing on an `artifact-piece__spin` inside it; don't animate the piece element itself. The styles are in `Styles/Inventory.uss`; the slots are drawn in USS (a thin square outline, `.inventory-slot`), without a texture.
