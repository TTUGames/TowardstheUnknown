...
Code reads a UI color from a custom property rather than hard-coding it: `CutShape` reads `--fill-color`, `--line-color`... and `ArtifactPiece` its outline's `--piece-line-color` (`CustomStyleResolvedEvent`, `customStyle.TryGetValue`). The rarity colors are not tokens: they come from `RarityPalette`, shared with the relics (see [inventory](inventory.md#pieces)). The wipe of `ScreenFade` and `SceneTransition` takes its look from the `slanted-wipe` class.
...
| `ScreenFade` | The room transition: covers the screen with the `SlantedWipe` named `Fade` (`FadeIn` ends once covered, `FadeOut` once revealed and hidden); `Map` loads the next room while it covers |
...
- `SlantedWipe`, the screen transition: five horizontal bands cut at 45° sweep in from the left one after the other (a comb, not one diagonal) until they cover the element, then sweep out on the right. A line can run on their moving edge (`--line-color`, `--line-width`); the game's wipe has none, its bands are plain. Drawn with the painter (`generateVisualContent`), no blur. `Cover(deltaTime)` and `Reveal(deltaTime)` are coroutines ending on the exact covered and hidden states; it blocks the pointer while shown and is not displayed once revealed. Look from USS custom properties, set by the `slanted-wipe` class of `Common.uss`: `--fill-color` (`--color-fade`), `--line-color` (`--color-accent`), `--line-width`, `--wipe-duration` (a time, `--duration-wipe`). `ScreenFade` (the room changes, in the 16:9 area, game time) and `SceneTransition` (the scene loads, whole screen, unscaled time) use it.
