...
| Animation | `animationClip` (played by the caster's `EntityAnimator`, none if empty), `followUpClip` (its second part, e.g. the Golem's shock wave jump then landing), `animationSpeed`, `duration` (time the next actions wait), `impactDelay` (from the start of the animation to the moment the effects apply, 0.5 s by default), `vfx` (`VFXInfo` list), `sound` (Wwise event) |

`Ability` is the runtime base, holding the range and area searches: `CanTarget(tile)`, `CanReach(from, tile)`, `GetTargets(tile)` and `Cast(caster, tile, chained, chainedRecovery)`; `IsAreaOfEffect` tells an area from a single target. `Cast` picks the targets, turns the caster towards the tile, posts the sound and queues, in order: an `AttackAnimationAction` (animation and VFX, done after `impactDelay`), the effects of `castEffects` then of `effects` on each target, and an `AttackRecoveryAction` that waits for the rest of `duration`, then removes the VFX. When the optional `chained` returns true (a [queued cast](entities.md#cast-queue) follows), the recovery ends after `chainedRecovery` seconds so that the next cast starts, and the VFX of the cut attack still play out their `duration` before `ReleaseVFX`. The hits, damage numbers and deaths thus land at the impact, not when the attack starts. No animation carries a hit event: tune `impactDelay` on the asset to match its animation and VFX.

### Artifacts

`ArtifactData` (`Assets/Data/Artifacts`) adds the skill bar icon (also drawn on its inventory piece), rarity, cost, uses per turn (0 for unlimited), cooldown, the player's neon color and weapon while casting, and the inventory shape with the placement of the icon on the piece ([inventory pieces](inventory.md#pieces)). Its asset name is its ID, key of its localized texts. `Artifact` is its runtime instance (`data.CreateArtifact()`), holding the cooldown and remaining uses. `Pay` takes the energy, a use and, on the last use, starts the cooldown; `Refund` gives them back (the cooldown only if that payment started it) for a [queued cast](entities.md#cast-queue) dropped before it plays. `PlayerAttack` pays, then calls `Cast`, at once or later.
...
`VFXWarmup.Warm` plays VFX once out of sight before they show: it takes their instances from the pool, simulates them and renders them with a hidden camera (a 128 px texture far below the rooms), which compiles their shaders and fills the pool; the first hit of an effect would otherwise freeze the game. It is called by `Room.Init` (the patterns of the room's enemies, from `EnemyAI.AllPatterns`, the player's artifacts and the `EntityFeedback.hitVFX` of the room's entities and the player), by `Collectable.SetArtifacts` (a chest's artifacts, before the player can take them) and by `InventoryManager` when the grid changes. Each prefab is warmed once per scene.

The spells' particle shaders are hand-written URP HLSL, unlit, in the transparent queue ("Towards the Unknown/VFX/..."): they read the particle systems' vertex streams (the color, uv0 with the custom data in zw, uv1, uv3) through `Rendering/VFX/VFXParticles.hlsl`. The center glow family (panning texture bent by a flow texture, noise, center glow shrinking with the custom data; added, blended, strong, or faded by a fresnel) shares `Rendering/VFX/CenterGlow.hlsl`, the dissolving one `Rendering/VFX/DissolveNoise.hlsl`; the others (DragonStrike's aura, ground wave and wind, WithoutFear's cutout, glow and distortion) sit next to their effect in `Art/VFX`. They were ported from Amplify with the same properties and keywords, so the materials kept their values. Soft particles are left out, as the Amplify shaders never faded under URP; WithoutFear's distortion reads the opaque texture (URP runs no GrabPass).
