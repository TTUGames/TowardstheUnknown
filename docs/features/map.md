...
Water pools are `WaterVolume`s (`Visuals`, usually through the `Environment/Water_Map4` or `Water_Drareg` prefab): a box of tiles, one tile high and scaled by the transform, whose mesh (foam in the vertex color, UVs offset by the world position) is built at load for the `Art/Water/Water.shadergraph` materials. The graph moves the top face with the wave of `Rendering/Water.hlsl` (`WaterWave`, through a Custom Function node: `_WaveSpeed`, `_WaveFrequency`, `_WaveScale`), the function the floating plants use too (see [vegetation](#vegetation)): a lily pad whose material has the same three values as the pool under it rises and tilts with its surface.
