...
`Prefabs/Environment/Snow.prefab`, placed once in the game and test scenes, holds the particles shared by every room (the rooms are loaded at the same place): the falling snow, glowing motes drifting up (the Motes object, additive) and a thin mist on the ground (the Mist object, additive, horizontal billboards). Their URP particle materials are in `Art/Materials/Environment`, with generated textures in `Art/Textures/Environment` (a soft flake for the snow, a glow with a hot core for the motes). The LowPolyCavePack's FX materials use legacy shaders that URP doesn't render: build URP materials from their textures instead. The flames of `FireTorch` and `FireCandle` flicker through their light's `LightFlicker`. The antechamber holds its own global volume (`Prefabs/Volume/Global Volume Antechamber`, priority 1): a warmer, more contrasted grade over the game's, active while the room is.
