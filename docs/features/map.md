...
The flames of `FireTorch` and `FireCandle` flicker through their light's `LightFlicker`. The antechamber holds its own global volume (`Prefabs/Volume/Global Volume Antechamber`, priority 1): a warmer, more contrasted grade over the game's, active while the room is.
