...
`RoomInfo` remembers whether a room was visited, its spawn layout and its unclaimed loot.
...
1. `RoomInfo.LoadRoom` instantiates it; `Room.SetExits` removes the exits leading nowhere and adds the exit VFX on the others; `Room.Init` registers the player in the turn system, spawns the layout (first visit only) and the remaining loot, and raises `GameEvents.RoomEntered`.
...
Stopping on an exit (`TransitionTile`) out of combat raises `GameEvents.RoomLeft`, fades out, destroys the room and enters the adjacent one. The minimap (`MinimapPanel`) follows the current room.
