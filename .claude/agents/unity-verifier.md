...
3. Playtest with `.claude/skills/unity-playtest/scripts/playtest.sh`, following `.claude/skills/unity-playtest/SKILL.md`: choose the scene (`RoomTestScene` for a combat, `2-Game` for the map, rooms and inventory, `FixGeneration` for the boss, east of the spawn room, `RoomGallery` to walk through every room without enemies), exercise the changed behavior and the flows around it (deploy, move, cast, enemy turns, end of combat, room change, inventory), and read `Probe.Status` / `Probe.Player` after each step.
