...
- Nothing is loaded from `Resources` by path except `GameAssets` (`Resources/GameAssets`), which references the prefabs, VFX and sprites needed by code without a scene object, and the localization JSON. Reference new assets from a serialized field or from `GameAssets` rather than adding `Resources.Load` calls.
